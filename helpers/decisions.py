...
class DecisionStore:
    ...
    def transaction(self, scope):
        ...
        try:
            ...
            connection.execute('INSERT OR IGNORE INTO gate_sessions(scope) VALUES (?)',
    ...
    def _load(self, connection, scope, decision_id):
        row = connection.execute('SELECT * FROM gate_decisions WHERE id=? AND scope=?',
                                 (self.digest(decision_id), self.scope_key(scope))).fetchone()
        if row is None:
            raise DecisionError('decision_not_found')
    ...
    def create(self, scope, payload):
        ...
        with self.transaction(scope) as db:
            db.execute('DELETE FROM gate_decisions WHERE scope=? AND expires<=?',
                       (self.scope_key(scope), now))
            count = db.execute('SELECT count(*) FROM gate_decisions WHERE scope=?',
                               (self.scope_key(scope),)).fetchone()[0]
            if count >= 128:
                raise DecisionError('decision_quota_exceeded')
            db.execute('INSERT INTO gate_decisions(id,scope,expires,payload) VALUES(?,?,?,?)',
                       (self.digest(identifier), self.scope_key(scope), now + 300000000,
    ...
    def update(self, scope, decision_id, payload):
        with self.transaction(scope) as db:
            ...
            db.execute('UPDATE gate_decisions SET payload=? WHERE id=?',
                       (self._seal(scope, decision_id, payload), self.digest(decision_id)))

    def disabled(self, scope):
        with self.transaction(scope) as db:
            return db.execute('SELECT disabled FROM gate_sessions WHERE scope=?',
                              (self.scope_key(scope),)).fetchone()[0] != 0
    ...
    def claim(self, scope, decision_id, limit, expected_payload):
        ...
        with self.transaction(scope) as db:
            ...
            row = db.execute('SELECT * FROM gate_sessions WHERE scope=?',
                             (self.scope_key(scope),)).fetchone()
            if row['disabled'] != 0:
            ...
            db.execute('UPDATE gate_sessions SET calls=calls+1, active=? WHERE scope=?',
                       (self.digest(decision_id), self.scope_key(scope)))

    def finish(self, scope, decision_id):
        with self.transaction(scope) as db:
            db.execute('UPDATE gate_sessions SET active=NULL WHERE scope=? AND active=?',
                       (self.scope_key(scope), self.digest(decision_id)))

    def revoke_scope(self, scope):
        with self.transaction(scope) as db:
            db.execute('UPDATE gate_sessions SET disabled=1 WHERE scope=?',
                       (self.scope_key(scope),))
            db.execute('DELETE FROM gate_decisions WHERE scope=?', (self.scope_key(scope),))

    def invalidate(self, scope, decision_id):
        """A changed boundary requires a new decision even if upstream later reverts."""
        with self.transaction(scope) as db:
            db.execute('UPDATE gate_decisions SET used=1 WHERE scope=? AND id=?',
                       (self.scope_key(scope), self.digest(decision_id)))
