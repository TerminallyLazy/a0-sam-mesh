...
from .capabilities import EXPECTED_READ_TOOL_SCHEMAS, CALL_REMOTE_TOOL_SCHEMA, _schema_matches
...
class RemoteTools:
    ...
    # unchanged risk metadata: guarded invocation is unavailable on these revisions.
    ...
    def check_transport(self):
        if self.client.mcp._config.type != 'uds':
    ...
    async def _check_schema(self, name):
        ...
        expected = (CALL_REMOTE_TOOL_SCHEMA if name == 'call_remote_tool'
    ...
    async def read(self, name, arguments):
        ...
        allowed_count = (2 if name == 'discover_remote_services'
                         and arguments.get('type') == 'inference' else 1)
        if (not 1 <= len(result.content) <= allowed_count
    ...
    async def describe(self, peer_id, tool_name):
        ...
        value = await self.read('describe_remote_tool',
                                {'peer_id': peer_id, 'tool_name': tool_name})
        required = {'peer_id', 'tool_name', 'description', 'input_schema'}
        if (not isinstance(value, dict) or not required <= value.keys()
                or value.keys() - required - {'output_schema'}
                or value['peer_id'] != peer_id or value['tool_name'] != tool_name
        ...
        return ToolDescriptor(tool_name, peer_id, 'mcp://' + parsed.netloc,
                              value['description'], value['input_schema'],
                              value.get('output_schema'), (), datetime.now(UTC).isoformat(),
    ...
    async def call(self, descriptor, arguments, labels):
        ...
        return await self.client.call_mcp_tool('call_remote_tool', {
            'peer_id': descriptor.peer_id, 'tool_name': descriptor.canonical_uri,
            'arguments': arguments, 'required_labels': labels})
