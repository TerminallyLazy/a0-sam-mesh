...
class PinnedUnixBackend(AnyIOBackend):
    async def connect_unix_socket(self, path, timeout=None, socket_options=None):
        ...
        try:
            ...
            parts = path.split('/')[1:]
            for part in parts[:-1]:
                parent = os.open(part, os.O_PATH | os.O_DIRECTORY | os.O_NOFOLLOW,
                                 dir_fd=parent)
                descriptors.append(parent)
            ...
            return await super().connect_unix_socket(
                f'/proc/self/fd/{fd}', timeout, socket_options)
        except Exception:
...
class PinnedUnixTransport(httpx.AsyncHTTPTransport):
    def __init__(self, path):
        ...
        self._pool = httpcore.AsyncConnectionPool(
            uds=path, retries=0, network_backend=PinnedUnixBackend(),
