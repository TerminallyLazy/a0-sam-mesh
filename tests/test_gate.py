...
class GateAvailabilityTests(unittest.TestCase):
    def test_gate_exists(self):
        self.assertIsNotNone(
            importlib.util.find_spec("helpers.gate"), "Task 6 needs an executable DelegationGate"
        )
...
class GateTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.config = ResolvedConfig(
            ...
            FeatureFlags(True, False, False, False),
        ...
        self.gate = DelegationGate(
            lambda: self.config, self.adapter, self.store, self.leases, self.audit
        )

    async def preflight(self, args=None):
        return await self.gate.preflight(
            self.tool.peer_id,
            self.tool.canonical_uri,
    ...
    async def test_current_scope_passport_feature_route_labels_changes_deny(self):
        for change in (
            ...
            lambda c: replace(
                ...
                passport=replace(
                    c.passport,
                    inference=replace(c.passport.inference, required_labels=("region=eu",)),
                ),
            ),
        ):
    ...
    async def test_schema_annotations_peer_service_and_arguments_invalidate(self):
        ...
        for tool in (
            ...
            replace(original, peer_id="other"),
            replace(original, service="mcp://other"),
    ...
    async def test_exact_five_minute_expiry(self):
        ...
        self.assertEqual(
            (await self.gate.invoke(decision.decision_id)).error_code, "approval_expired"
        )
        self.assertEqual(self.calls, 0)
    ...
    async def test_concurrency_quota_and_emergency(self):
        ...
        results = await asyncio.gather(
            self.gate.invoke(first.decision_id), self.gate.invoke(first.decision_id)
        )
        self.assertEqual(sum(r.error_code is None for r in results), 1)
        self.assertEqual(
            (await self.gate.invoke(second.decision_id)).error_code, "session_quota_exceeded"
        )
        self.gate.emergency_disconnect()
        self.assertEqual(
            (await self.gate.invoke(second.decision_id)).error_code, "emergency_disconnected"
        )
        self.assertEqual(self.calls, 1)
...
class AdapterContracts(unittest.IsolatedAsyncioTestCase):
                    ...
                    if params["name"] == "describe_remote_tool":
                        ...
                        return self._json(
                            200,
                            {
                                ...
                                "id": payload["id"],
                                "result": {
                                    "content": [{"type": "text", "text": json.dumps(description)}]
                                },
                            },
                        )
            ...
            async with Sidecar(uds_path=root + "/node.sock") as server:
                ...
                async with SamClient(cfg) as client:
                    ...
                    descriptor = await adapter.describe(
                        "peer-finance", "mcp://finance/update-record"
                    )
                    self.assertEqual(descriptor.canonical_uri, "mcp://finance/update-record")
                    self.assertFalse(
                        adapter.risk_metadata_verified,
    ...
    async def test_adapter_denies_unverified_tcp_before_credentials_or_discovery(self):
        from helpers.remote_tools import RemoteTools
        from helpers.sam_client import SamClient
        from tests.fakes.sam_sidecar import FakeSamSidecar
        ...
        async with FakeSamSidecar() as server:
            ...
            async with SamClient(cfg) as client:
                # This test keeps the unverified-transport denial invariant.
                import httpx

                await client.mcp._client._transport.aclose()
                client.mcp._client._transport = httpx.AsyncHTTPTransport()
                with self.assertRaises(Exception):
...
class GateHardeningTests(GateTests):
    ...
    async def test_session_busy_does_not_consume_other_decision(self):
        ...
        while not self.calls:
            await asyncio.sleep(0)
        self.assertEqual(
            (await self.gate.invoke(second.decision_id)).error_code, "invocation_in_progress"
        )
        self.waiter.set()
        await task
...
class AdditionalAdapterContracts(unittest.IsolatedAsyncioTestCase):
        ...
        with tempfile.TemporaryDirectory() as root:
            async with Sidecar(uds_path=root + "/node.sock") as server:
                async with SamClient(
                    TransportConfig("uds", server.base_url, server.uds_path, None)
                ) as client:
                    self.assertEqual(
                        await RemoteTools(client).read(
    ...
    async def test_remote_call_schema_checked_before_description(self):
        ...
        from helpers.remote_tools import RemoteTools
        from helpers.sam_client import SamClient
        from tests.fakes.sam_sidecar import CURRENT_TOOLS, FakeSamSidecar
        ...
        with tempfile.TemporaryDirectory() as root:
            async with FakeSamSidecar(uds_path=root + "/node.sock", tools=tools) as server:
                ...
                calls = [
                    ...
                    if isinstance(r["json"], dict) and r["json"].get("method") == "tools/call"
                ]
...
class FinalGateContracts(GateTests):
    ...
    async def test_process_key_loss_denies_persisted_decisions(self):
        ...
        with patch("helpers.decisions._CIPHER", Fernet(Fernet.generate_key())):
            self.assertEqual(
                (await self.gate.invoke(decision.decision_id)).error_code, "decision_unavailable"
            )
        self.assertEqual(self.calls, 0)
